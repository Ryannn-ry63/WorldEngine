"""CFPI method-neutral pre-action observation and one-step intervention manager."""
...
class DiffusionDriveCFPICausalCacheManager(DiffusionDrivePreActionOracleManager):
    ...
    def __init__(self):
        ...
        self.intervention_mode = str(
            config.get("diffusiondrive_cfpi_intervention_mode", "observe_only")
        )
        if self.intervention_mode not in {"observe_only", "one_shot_manifest"}:
            raise ValueError(f"invalid CFPI intervention mode: {self.intervention_mode}")
        self.collection_id = str(
            config.get("diffusiondrive_cfpi_collection_id", "") or ""
        )
        if not self.collection_id:
            raise ValueError("CFPI collection requires a nonempty collection id")
        self.behavior_train_seed = int(
            config.get("diffusiondrive_behavior_train_seed", -1)
        )
        if self.behavior_train_seed != 0:
            raise ValueError("CFPI causal cache freezes scalar V3 train seed 0")
        self.target_manifest_path = str(
            config.get("diffusiondrive_cfpi_treatment_manifest", "") or ""
        )
        expected_sha = str(
            config.get("diffusiondrive_cfpi_treatment_manifest_sha256", "none")
        )
        ...
        if self.intervention_mode == "observe_only":
            if self.target_manifest_path or expected_sha != "none":
                raise ValueError(
                    "CFPI observe-only collection must not receive a treatment manifest"
                )
        else:
            if not self.target_manifest_path or expected_sha == "none":
                raise ValueError("CFPI intervention requires a frozen treatment manifest")
            path, payload = cfpi.load_treatment(
                self.target_manifest_path, expected_sha
            )
            if str(payload["collection_id"]) != self.collection_id:
                raise RuntimeError("CFPI collection/treatment id drifted")
            ...
            if len(self.targets) != len(rows):
                raise RuntimeError("duplicate CFPI treatment target scene")
    ...
    def _write_cfpi_record(
        ...
        record.update(
            schema_version=1,
            record_type="diffusiondrive_cfpi_causal_record",
            ...
            cfpi_collection_id=self.collection_id,
            cfpi_treatment_kind=self.treatment_kind,
            ...
            candidate_reward_valid_mask=np.ones(
                cfpi.NUM_CANDIDATES, dtype=np.bool_
            ),
        ...
        output_dir = (
            ...
            / "diffusiondrive_cfpi_causal_records"
        )
        output_dir.mkdir(parents=True, exist_ok=True)
        final_path = output_dir / (
            f"{sidecar['scene_prefix']}_{sidecar['planner_step']}_cfpicausal.pkl"
        )
        ...
        self.preaction_paths_df.to_csv(
            Path(self.engine.global_config["data_output_dir"])
            / "diffusiondrive_cfpi_causal_records.csv",
            index=False,
        )
    ...
    def before_step(self):
        ...
        if decision_step != self.current_step + 1:
            raise RuntimeError(
                f"CFPI pre-action step drift: state={self.current_step} "
        ...
        for key, value in expected.items():
            if contract.get(key) != value:
                raise RuntimeError(f"CFPI sidecar contract drifted at {key}")
        ...
        if (
            ...
            raise RuntimeError("invalid CFPI candidate set")
        ...
        if policy_index != int(np.argmax(logits)):
            raise RuntimeError("CFPI policy index/logit argmax drifted")
        if (
            r15.max_abs_error(
                sidecar["deployed_trajectory"], local_40[policy_index]
            )
            > cfpi.ACTION_TOLERANCE
        ):
            raise RuntimeError("CFPI sidecar/policy candidate parity drifted")
        ...
        if policy_action_error > cfpi.ACTION_TOLERANCE:
            raise RuntimeError(
                f"CFPI external policy action parity failed: {policy_action_error}"
            )
        ...
        if target is not None and decision_step == target_step:
            ...
            if any(value > cfpi.ARRAY_TOLERANCE for value in errors.values()):
                raise RuntimeError(f"CFPI frozen target context drifted: {errors}")
            treatment_index = int(target["treatment_index"])
            if treatment_index not in range(cfpi.NUM_CANDIDATES):
                raise RuntimeError("CFPI frozen treatment index is invalid")
        ...
        if deployed_action_error > cfpi.ACTION_TOLERANCE:
            raise RuntimeError("CFPI deployed action replacement failed")
        self._write_cfpi_record(
            sidecar,
            sidecar_path,
            scores,
            components,
        ...
        logger.info(
            "CFPI causal scene=%s decision=%d collection=%s applied=%s "
    ...
    def step(self):
        ...
        if self.current_scene is None:
            raise RuntimeError("no scene available for CFPI causal cache manager")
