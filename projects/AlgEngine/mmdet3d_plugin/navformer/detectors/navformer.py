...
class NAVFormer(MVXTwoStageDetector):
        ...
        **kwargs,  # [1, 9]
        ...
        for batch_idx in range(b):
            ...
            if all_traj_40 is not None:
                pdm_dict["all_trajectories"] = all_traj_40[batch_idx].detach().float().cpu().numpy()
    ...
    def _forward_test_rollout(self, plan_results, img_metas, sdc_planning, batch_size):
        ...
        for batch_idx in range(batch_size):
            ...
            pdm_dict["ade_4s"] = np.mean(np.linalg.norm(pred_traj - gt_traj, axis=-1))
            pdm_dict["fde_4s"] = np.linalg.norm(pred_traj[-1] - gt_traj[-1])
            return_list.append(pdm_dict)
