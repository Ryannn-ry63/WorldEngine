...
prepare = importlib.import_module("prepare_grpo_selector_v3_rare_log_data")
smoke_subset = importlib.import_module(
    "prepare_grpo_selector_v3_rare_log_smoke_subset"
)
trainer = importlib.import_module("train_grpo_selector_v3_cached_rare_log")
...
def test_rare_union_and_same_log_common_pairing(small_full_navtrain, tmp_path):
    ...
    for row in pairs:
        rare_log = small_full_navtrain["token_logs"][row["rare_token"]]
        common_log = small_full_navtrain["token_logs"][row["common_token"]]
        assert rare_log == common_log == row["log_name"]
...
def test_pairing_fails_when_a_rare_log_has_no_strict_common(
    small_full_navtrain, tmp_path
):
    rows = {
        "a0": component_row(naf=0.0, progress=0.0),
