#!/usr/bin/env python3
"""Train selector architecture controls on audited rare/same-log-common pairs."""
...
METHOD_PREFIX = "trajectory_set_reasoner_exact_group_grpo_v1"
FORMAL_NOISE_SEEDS = (0, 1, 2)
FORMAL_EPOCHS = 64
FORMAL_CHECKPOINT_EPOCHS = (16, 32, 64)
FORMAL_EXAMPLES_PER_CACHE_EPOCH = 6339
FORMAL_BATCH_SIZE = 64
FORMAL_TOTAL_EXAMPLES = 1217088
FORMAL_OPTIMIZER_STEPS = 19200
...
def parse_args():
    ...
    parser.add_argument(
        "--checkpoint-epochs", default=",".join(map(str, FORMAL_CHECKPOINT_EPOCHS))
    )
    ...
    parser.add_argument("--num-relation-layers", type=int, default=2)
    parser.add_argument("--num-set-layers", type=int, default=1)
    parser.add_argument("--allow-partial-coverage", action="store_true")
    parser.add_argument(
        "--formal-contract",
        action="store_true",
        help="enforce the frozen 64-epoch architecture-study contract",
    )
...
def load_pair_contract(pair_manifest, rare_data_audit):
    ...
    if (
        ...
        or audit.get("method") != "diffusiondrive_v3_full_navtrain_rare_original_v1"
    ):
...
def expected_method(sampling_mode, architecture, ablation):
    if sampling_mode not in {"rare_balanced", "paired_common"}:
        raise ValueError(f"unsupported sampling mode: {sampling_mode}")
    return "_".join(
        (METHOD_PREFIX, architecture, ablation, sampling_mode)
    )
...
def validate_formal_contract(args):
    expected = {
        ...
        "method_name": expected_method(
            args.sampling_mode, args.architecture, args.ablation
        ),
    ...
    if drift:
        raise RuntimeError("formal trajectory-set contract drifted: " + repr(drift))
...
def load_caches(paths):
    if len(paths) != 3:
        raise RuntimeError("architecture training requires exactly three train caches")
...
def main():
    ...
    if args.method_name is None:
        args.method_name = expected_method(
            args.sampling_mode, args.architecture, args.ablation
        )
    ...
    if min(args.num_temporal_layers, args.num_relation_layers, args.num_set_layers) < 0:
        raise ValueError("architecture layer counts must be non-negative")
    ...
    if args.formal_contract and checkpoint_epochs != FORMAL_CHECKPOINT_EPOCHS:
        raise RuntimeError(
            "formal checkpoint schedule drifted: " + repr(checkpoint_epochs)
        )
    ...
    if args.sampling_mode == "rare_balanced":
        ...
        minimum_per_class = args.epochs * args.examples_per_cache_epoch
    if len(pair_rows) > minimum_per_class and not args.allow_partial_coverage:
        raise RuntimeError(
    ...
    if args.architecture == "trajectory_set_reasoner":
        architecture_config = {
            "num_temporal_layers": args.num_temporal_layers,
            "num_relation_layers": args.num_relation_layers,
        }
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for cache_index in cache_order:
            if args.sampling_mode == "rare_balanced":
                ...
                rare_count, common_count = 0, args.examples_per_cache_epoch
                selected = []
    ...
    if (
        args.sampling_mode == "rare_balanced"
        and class_examples["rare"] != class_examples["common"]
    ):
        raise RuntimeError("global rare/common sampling balance drifted")
    if (
        args.sampling_mode == "paired_common"
        and class_examples != {"rare": 0, "common": total_examples}
    ):
    ...
    for cache_index in range(len(caches)):
        ...
        for class_name in sampled_classes:
            ...
            if len(sampler.visited) != len(pair_rows) and not args.allow_partial_coverage:
                raise RuntimeError(
    ...
    report = {
        ...
        "sampling": {
            "contract": (
                ...
                "V3 6339-example epoch budget; sampling_mode="
                + args.sampling_mode
            ),
            "examples_per_cache_epoch": args.examples_per_cache_epoch,
    ...
    print(f"PASS trajectory-set selector training: {report_path}")
