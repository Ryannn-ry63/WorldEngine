#!/usr/bin/env python3
"""Fail-closed audit for one V4 baseline or causal intervention collection."""
...
def record_paths(root: Path, layout: str) -> list[Path]:
    suffix = "WE_output/openscene_format/diffusiondrive_v4_causal_records"
    if layout == "merged":
        paths = sorted((root / suffix).glob("*_v4causal.pkl"))
    else:
        paths = sorted(root.glob(f"split_*/{suffix}/*_v4causal.pkl"))
    if not paths:
        raise RuntimeError(f"no {layout} V4 causal records under {root}")
    return paths
...
def validate_record(
    ...
    if (
        ...
        or row.get("record_type") != "diffusiondrive_v4_causal_record"
    ):
        raise RuntimeError(f"V4 causal record schema drifted: {path}")
    if row.get("selector_rollout_contract") != contract:
        raise RuntimeError(f"V4 rollout contract drifted: {path}")
    if (
        row.get("checkpoint_sha256") != checkpoint_sha
        or row.get("candidate_noise_namespace") != namespace
    ):
        raise RuntimeError(f"V4 checkpoint/noise provenance drifted: {path}")
    expected_flat = {
        "behavior_policy_family": "scalar_v3",
        "behavior_policy_train_seed": 0,
        ...
        "v4_collection_id": contract["collection_id"],
    }
    for key, expected in expected_flat.items():
        if row.get(key) != expected:
            raise RuntimeError(f"V4 flat provenance {key} drifted: {path}")
    ...
    if valid.shape != (20,) or not valid.all():
        raise RuntimeError(f"V4 candidate validity drifted: {path}")
    if tuple(row.get("reward_component_names", ())) != metrics_common.COMPONENT_NAMES:
        raise RuntimeError(f"V4 reward component order drifted: {path}")
    ...
    if int(row["worldengine_step"]) != state or decision != state + 1:
        raise RuntimeError(f"V4 pre-action timing drifted: {path}")
    ...
    if (
        ...
        raise RuntimeError(f"V4 policy index export drifted: {path}")
    ...
    if int(row["local_reward_oracle_index"]) != reward_oracle:
        raise RuntimeError(f"V4 local reward oracle drifted: {path}")
    ...
    if int(row["treatment_index"]) != expected_treatment:
        raise RuntimeError(f"V4 treatment index drifted: {path}")
    if bool(row["intervention_applied"]) != expected_applied:
        raise RuntimeError(f"V4 intervention schedule drifted: {path}")
    expected_deployed = expected_treatment if expected_applied else policy
    if int(row["deployed_index"]) != expected_deployed:
        raise RuntimeError(f"V4 deployed index drifted: {path}")
    ...
    if at_target:
        ...
        if any(value > common.ARRAY_TOLERANCE for value in errors.values()):
            raise RuntimeError(f"V4 target context drifted: {path}: {errors}")
        ...
        if not np.isclose(
            ...
            raise RuntimeError(f"V4 target local reward diagnostic drifted: {path}")
    elif row.get("target_reward_max_abs_error") is not None:
        raise RuntimeError(f"V4 target diagnostic leaked outside target frame: {path}")
    ...
    if parity > common.ACTION_TOLERANCE:
        raise RuntimeError(f"V4 action parity failed: {path}")
    expected_scalars = {
    ...
    for key, expected in expected_scalars.items():
        if not np.isclose(float(row[key]), float(expected), rtol=0.0, atol=1e-6):
            raise RuntimeError(f"V4 {key} drifted: {path}")
    for provenance in ("raw_observation_path", "sidecar_path"):
        if not Path(str(row.get(provenance, ""))).is_file():
            raise RuntimeError(f"missing V4 provenance {provenance}: {path}")
...
def audit_collection(args) -> dict:
    ...
    if args.mode == "observe_only":
        if args.treatment_manifest is not None or args.treatment_sha256 != "none":
            raise RuntimeError("V4 baseline must not receive a treatment manifest")
    else:
        if args.treatment_manifest is None:
            raise RuntimeError("V4 intervention is missing its treatment manifest")
        ...
        if treatment["collection_id"] != args.collection_id:
            raise RuntimeError("V4 treatment collection id drifted")
        if treatment["source_split"] != args.source_split:
            raise RuntimeError("V4 treatment source split drifted")
    ...
    if args.mode == "one_shot_manifest" and set(targets) != expected_scenes:
        raise RuntimeError("V4 treatment/scenario coverage differs")
    ...
    for path in record_paths(root, args.layout):
        ...
        if scene not in expected_scenes:
            raise RuntimeError(f"V4 record outside scenario contract: {path}")
        key = (scene, decision)
        if key in seen:
            raise RuntimeError(f"duplicate V4 logical frame: {key}")
        seen.add(key)
        ...
        if len(parts) != 1:
            raise RuntimeError(f"ambiguous V4 worker provenance: {path}")
        worker = parts[0]
        workers.add(worker)
    ...
    if bad or set(steps) != expected_scenes:
        raise RuntimeError(
            f"V4 record coverage failed: bad={bad} "
            f"missing={sorted(expected_scenes-set(steps))}"
        )
    if len(workers) != args.expected_workers:
        raise RuntimeError(
            f"expected {args.expected_workers} V4 workers, got {sorted(workers)}"
        )
    if len(config_shas) != 1 or "None" in config_shas:
        raise RuntimeError("V4 config provenance drifted")
    if code_shas != {args.expected_code_sha}:
        raise RuntimeError("V4 code provenance drifted")
    if any(len(value) != 1 for value in resolved.values()):
        raise RuntimeError("V4 resolved config drifted within worker")
    ...
    if len(interventions) != expected_interventions:
        raise RuntimeError(
            f"V4 intervention count drifted: {len(interventions)} != {expected_interventions}"
        )
    ...
    if args.metrics_csv is not None:
        ...
        if set(outcomes) != expected_scenes:
            raise RuntimeError("V4 metric scenario coverage drifted")
        outcome = {
