...
def main():
    ...
    is_scene_v3 = cfg.model.planning_head.get("scene_selector") is not None
    if is_scene_v3 and objective != "exact_group_grpo":
        raise RuntimeError("V3 scene selector requires the exact group objective")
    ...
    if objective == "exact_group_grpo":
        if contract.get("fixed_ratio_clip", True):
            raise RuntimeError("exact-group objective must disable fixed clipping")
        if contract.get("policy_objective") != (
            "exact_complete_action_expected_advantage"
        ):
            raise RuntimeError("exact-group objective contract drifted")
    ...
    if not is_scene_v3 and contract.trainable_selector_tensors != 10:
        raise RuntimeError("formal config must train exactly 10 selector tensors")
    if is_scene_v3:
        if contract.get("architecture") != "scene_conditioned_residual_set_selector_v3":
            raise RuntimeError("V3 scene-selector architecture contract drifted")
        if not contract.get("residual_on_frozen_reference", False):
            raise RuntimeError("V3 must initialize as a residual on pi_ref")
        if contract.get("gate", True):
            raise RuntimeError("a deployment gate leaked into V3")
        if contract.get("reward_components_as_inputs", True):
            raise RuntimeError("reward components leaked into V3 selector inputs")
    ...
    if not trainable or optimized_ids != trainable_ids:
        raise RuntimeError("optimizer/trainable selector parameter sets drifted")
    expected_prefix = (
        "planning_head.scene_selector." if is_scene_v3 else
        "planning_head.diff_decoder.layers.1.task_decoder.plan_cls_branch."
    )
    if not all(name.startswith(expected_prefix) for name in trainable):
        raise RuntimeError("a non-selector tensor is trainable")
    if not is_scene_v3 and len(trainable) != 10:
        raise RuntimeError(
            f"expected 10 frozen-baseline selector tensors, got {len(trainable)}"
        )
