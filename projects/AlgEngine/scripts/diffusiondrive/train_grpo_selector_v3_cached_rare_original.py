...
FORMAL_RARE_METHOD = "scene_conditioned_exact_group_grpo_v3_rare_original_v1"
FORMAL_PAIRED_COMMON_METHOD = (
    "scene_conditioned_exact_group_grpo_v3_paired_common_v1"
)
...
def parse_args():
    ...
    parser.add_argument(
        "--formal-contract",
        action="store_true",
        help="fail unless all V3 rare-original budget and hyperparameter constants match",
    )
...
def expected_method(sampling_mode):
    if sampling_mode == "rare_balanced":
        return FORMAL_RARE_METHOD
    if sampling_mode == "paired_common":
        return FORMAL_PAIRED_COMMON_METHOD
...
def validate_formal_contract(args):
    ...
    if drift:
        raise RuntimeError("formal V3 rare-original contract drifted: " + repr(drift))
...
def load_caches(paths):
    if len(paths) != 3:
        raise RuntimeError("V3 rare-original training requires exactly three train caches")
...
def main():
        ...
        for cache_index in cache_order:
            ...
            for start in range(0, len(order), args.batch_size):
                ...
                if not bool(torch.isfinite(loss)):
                    raise RuntimeError(
                        f"non-finite V3 rare-original loss at epoch {epoch}"
                    )
                loss.backward()
    ...
    print(f"PASS V3 rare-original cached selector training: {report_path}")
