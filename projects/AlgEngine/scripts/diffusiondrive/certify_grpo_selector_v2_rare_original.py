#!/usr/bin/env python3
"""One-shot certification of development-selected V2 rare-original policy."""
...
import evaluate_grpo_selector_v2_rare_original as evaluator
...
def main() -> None:
    ...
    if (
        ...
        != "diffusiondrive_v2_rare_original_development_selection_v1"
        or selection.get("certification_consumed")
    ):
        raise RuntimeError("invalid or already-consumed V2 selection")
    ...
    pooled = v3_evaluator.pooled(values_by_seed)
    rare_scenes = [str(caches[0]["scenes"][index]) for index in rare_indices.tolist()]
    ...
    report = {
        "schema_version": 1,
        "status": "PASS",
        "method": "diffusiondrive_v2_rare_original_certification_v1",
        ...
        "rare_vote_strata": v3_evaluator.vote_strata(values_by_seed, pair_rows),
