#!/usr/bin/env python3
"""Apply the frozen PCRA V2 offline gate without reading certification data."""
...
def load_labeled(inputs):
    ...
    for label, raw_path in inputs:
        if label in seen:
            raise RuntimeError(f"duplicate PCRA V2 label: {label}")
        seen.add(label)
...
def main():
    ...
    if proposal32.get("checkpoint_sha256") != PROPOSAL32_SHA256:
        raise RuntimeError("PCRA V2 proposal32 checkpoint SHA256 drifted")
    ...
    if len(candidates) != 1:
        raise RuntimeError("PCRA V2 predeclares exactly one promotable main arm")

    rows = []
    for row in candidates:
        candidate = row["checkpoint"]
        required = {
            "selector_architecture": ARCHITECTURE,
            "objective": OBJECTIVE,
            "counterfactual_loss": MAIN_LOSS,
        ...
        if drift:
            raise RuntimeError(f"PCRA V2 candidate contract drifted: {drift}")
        ...
        gates = {
            ...
            "real_rare_preserves_best_proposal_within_020": candidate["strata"][
                "real_rare"
            ]["current_reward"] >= proposal_stratum_floor["real_rare"] - 0.020,
            "synthetic_preserves_best_proposal_within_020": candidate["strata"][
                "synthetic"
            ]["current_reward"] >= proposal_stratum_floor["synthetic"] - 0.020,
    ...
    selected = rows[0] if rows[0]["eligible"] else None
    report = {
        "schema_version": 2,
        "status": "PASS",
        "method": "pcra_v2_frozen_offline_development_gate",
        ...
        "candidates": rows,
        "selected": selected,
        "selection_rule": "single_predeclared_pcra_v2_main" if selected else None,
