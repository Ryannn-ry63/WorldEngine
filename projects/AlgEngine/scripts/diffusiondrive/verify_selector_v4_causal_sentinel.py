#!/usr/bin/env python3
"""Verify that a policy-index V4 intervention exactly reproduces baseline V3."""
...
def verify(args) -> dict:
    ...
    if (
        ...
        raise RuntimeError("invalid V4 baseline for sentinel comparison")
    if (
        ...
        raise RuntimeError("invalid V4 policy sentinel collection")
    ...
    if (
        ...
        raise RuntimeError("invalid V4 policy sentinel treatment")
    if sentinel["treatment_manifest_sha256"] != common.sha256_file(treatment_path):
        raise RuntimeError("V4 sentinel treatment provenance drifted")
    keys = (
        "checkpoint_sha256",
    ...
    if any(baseline[key] != sentinel[key] for key in keys):
        raise RuntimeError("V4 baseline/sentinel provenance drifted")
    ...
    if set(sentinel_outcomes) != {row["scene_id"] for row in treatment["targets"]}:
        raise RuntimeError("V4 sentinel outcome coverage drifted")
    errors = {}
    ...
    return {
        ...
        "method": "diffusiondrive_selector_v4_policy_sentinel_gate_v1",
        "decision": (
            "AUTHORIZE_V4_PILOT64_CAUSAL_COLLECTION"
            if reproducible
            else "STOP_V4_SENTINEL_NOT_REPRODUCIBLE"
        ),
