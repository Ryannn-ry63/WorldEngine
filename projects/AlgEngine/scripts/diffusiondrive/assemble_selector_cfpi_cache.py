#!/usr/bin/env python3
"""Assemble one complete CFPI causal-value surface from 20 audited arms."""
...
def load_master(path: Path) -> tuple[Path, dict]:
    ...
    if (
        ...
        raise RuntimeError(f"invalid CFPI master manifest: {path}")
    ...
    if common.sha256_file(target_path) != payload["target_manifest_sha256"]:
        raise RuntimeError("CFPI master target provenance drifted")
    return path, payload
...
def load_arm_audit(
    ...
    if collection is None or int(collection["treatment_index"]) != index:
        raise RuntimeError(f"CFPI master is missing {collection_id}")
    ...
    if (
        ...
        raise RuntimeError(f"invalid CFPI arm audit: {audit_path}")
    metric_path = Path(audit["closed_loop_outcome"]["metrics_csv"])
    if common.sha256_file(metric_path) != audit["closed_loop_outcome"]["metrics_csv_sha256"]:
        raise RuntimeError("CFPI metrics changed after audit")
    outcomes = load_outcomes(metric_path)
    expected = {str(row["scene_id"]) for row in treatment["targets"]}
    if set(outcomes) != expected:
        raise RuntimeError(f"CFPI arm outcome coverage drifted: {collection_id}")
...
def load_context(target: dict) -> tuple[dict, dict]:
    ...
    if common.sha256_file(path) != target["source_record_a_sha256"]:
        raise RuntimeError(f"CFPI target source record drifted: {path}")
    ...
    if (
        ...
        raise RuntimeError("CFPI target/source record identity drifted")
    arrays = {
    ...
    if not np.allclose(
        ...
        raise RuntimeError("CFPI cache target trajectory drifted")
    if not np.allclose(
        ...
        raise RuntimeError("CFPI cache target logits drifted")
...
def assemble(args) -> tuple[Path, Path]:
    ...
    if audit_output.exists():
        if not output_path.exists():
            raise RuntimeError("CFPI cache audit exists without its cache")
        ...
        if (
            ...
            raise RuntimeError("existing CFPI cache assembly drifted")
        load_master(args.master_manifest)
        for entry in audit["arm_collection_audits"]:
            if (common.sha256_file(entry["path"]) != entry["sha256"]
                    or common.sha256_file(entry["metrics_csv"]) != entry["metrics_csv_sha256"]):
                raise RuntimeError("Audited CFPI arm changed after cache assembly")
    ...
    repeat_ids = set(target_payload["repeat_scene_ids"])
    targets = [
        row for row in target_payload["targets"] if row.get("stage") == args.stage
        or (args.stage == "repeat8" and row["scene_id"] in repeat_ids)
    ]
    if len(targets) != STAGE_COUNTS[args.stage]:
        raise RuntimeError(f"CFPI {args.stage} target count drifted")
    target_ids = [str(row["scene_id"]) for row in targets]
    if len(set(target_ids)) != len(target_ids):
        raise RuntimeError(f"duplicate CFPI {args.stage} targets")
    ...
    for index in range(common.NUM_CANDIDATES):
        ...
        if provenance is None:
            ...
            raise RuntimeError("CFPI arm collection provenance drifted")
        arm_audits.append({
    ...
    for target in targets:
        ...
        if (q.shape != (20,) or outcome_components.shape != (20, 3)
                or not np.isfinite(q).all() or not np.isfinite(outcome_components).all()
                or np.any((q < 0) | (q > 1))):
            raise RuntimeError("CFPI causal outcome shape drifted")
        row = {
            **context,
            "scene_id": scene_id,
            ...
            "time_stratum": target["time_stratum"],
            "fold": target["fold"],
            ...
            "branch_success": [bool(arm_outcomes[i][scene_id]["success"]) for i in range(20)],
            "branch_first_violation_steps": [
                arm_outcomes[i][scene_id]["first_violation_step"] for i in range(20)
            ],
    ...
    spans = np.asarray([
        float(np.max(row["branch_returns"]) - np.min(row["branch_returns"]))
        for row in rows
    ])
    gaps = np.asarray([
        float(np.max(row["branch_returns"]) - row["branch_returns"][row["policy_index"]])
        for row in rows
    ])
    audit = {
        "schema_version": common.SCHEMA_VERSION,
