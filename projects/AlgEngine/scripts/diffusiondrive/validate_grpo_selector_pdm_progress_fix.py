...
def main():
    ...
    if not bool(cfg.model.planning_head.online_reward.get("use_cuda_simulator", True)):
    ...
    # Exercise the exact formal-training path. eval() dispatches the GRPO head
    # to _forward_test, where online rewards are intentionally omitted.
    model.train()
    ...
    metric_cache_path = Path(cfg.model.planning_head.online_reward.metric_cache_path)
        ...
        for data in loader:
            ...
            for batch_index, token in enumerate(tokens):
                ...
                gated_candidate_count += int(
                    np.count_nonzero(
                        ~np.isclose(
                            expected_components[:, :2].prod(axis=1), 1.0
                        )
                    )
                )
