...
trap 'rc=$?; echo "FAIL rollout-v1 formal seed=${SEED} stage=${CURRENT_STAGE} line=${LINENO} command=${BASH_COMMAND} exit=${rc}" >&2; echo "persistent_log: ${LOG_FILE}" >&2' ERR

for file in "${SELECTION}" "${REFERENCE_SUMMARY}" "${DIFFUSIONDRIVE_GRPO_BASELINE}"     "${SCRIPT_DIR}/train_grpo_selector_v3_cached.py"     "${SCRIPT_DIR}/materialize_grpo_selector_v3.py"     "${SCRIPT_DIR}/audit_grpo_selector_v3_checkpoint.py"     "${SCRIPT_DIR}/run_grpo_selector_formal_table_h100.sh"; do
...
if [[ "${SEED}" == "0" ]]; then
    ...
    [[ -f "${CERT_ROOT}/report.json" ]] || { echo "rollout-v1 certification has not run" >&2; exit 1; }
else
    ...
    mkdir -p "${TRAIN_ROOT}"
    CUDA_VISIBLE_DEVICES=0 "${ALGENGINE_PYTHON}"         "${SCRIPT_DIR}/train_grpo_selector_v3_cached.py"         --train-cache "${CACHE_ROOT}/train_seed0/cache.pt"         --train-cache "${CACHE_ROOT}/train_seed1/cache.pt"         --train-cache "${CACHE_ROOT}/train_seed2/cache.pt"         --development-cache "${CACHE_ROOT}/development_seed3/cache.pt"         --development-cache "${CACHE_ROOT}/development_seed4/cache.pt"         --development-cache "${CACHE_ROOT}/development_seed5/cache.pt"         --output-dir "${TRAIN_ROOT}" --temperature "${TEMPERATURE}"         --learning-rate "${LEARNING_RATE}" --kl-weight "${KL_WEIGHT}"         --seed "${SEED}" --epochs "${EPOCH}" --checkpoint-epochs "${EPOCH}"         --batch-size 64 --device cuda --ablation full --method-name "${METHOD}"
    ...
    "${ALGENGINE_PYTHON}" "${SCRIPT_DIR}/materialize_grpo_selector_v3.py"         --baseline "${DIFFUSIONDRIVE_GRPO_BASELINE}"         --scene-selector-state "${SELECTOR_STATE}"         --expected-selector-sha256 "${SELECTOR_SHA}"         --expected-method "${METHOD}"         --release-name e2e_diffusiondrive_grpo_selector_rollout_v1         --output "${CHECKPOINT}" --manifest "${MANIFEST}"
fi
...
"${ALGENGINE_PYTHON}" "${SCRIPT_DIR}/audit_grpo_selector_v3_checkpoint.py"     --baseline "${DIFFUSIONDRIVE_GRPO_BASELINE}" --checkpoint "${CHECKPOINT}"     --output "$(dirname "${CHECKPOINT}")/checkpoint_audit.json"
...
"${SCRIPT_DIR}/run_grpo_selector_formal_table_h100.sh"     "${CHECKPOINT}" "${EXPECTED_SHA}" "${FORMAL_MODEL}" "${SEED}"     "same V3 selector-only exact-group GRPO trained on base-policy closed-loop rollout states; train seed=${SEED}; eval seed=${SEED}"
