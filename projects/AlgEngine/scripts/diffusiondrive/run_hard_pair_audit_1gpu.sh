#!/usr/bin/env bash
# Train-only fixed-top5 information audit. It reuses existing schema-v4 caches.
set -Eeuo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
. "${SCRIPT_DIR}/trajectory_set_reasoner_env.sh"

GPU_INDEX="${HARD_PAIR_GPU:-0}"
...
PRIOR_GATE="${WORLDENGINE_ROOT}/experiments/diffusiondrive/interaction_selector_v1/probe/probe_20260901T053452Z/probe_gate.json"
EXPERIMENT_ROOT="${WORLDENGINE_ROOT}/experiments/diffusiondrive/hard_pair_audit_v1"
RUN_ID="${HARD_PAIR_AUDIT_ID:-audit_$(date -u +%Y%m%dT%H%M%SZ)}"
...
for path in "${required[@]}"; do
    [[ -e "${path}" ]] || { echo "Missing hard-pair dependency: ${path}" >&2; exit 1; }
done
[[ "$(sha256sum "${V3_CHECKPOINT}" | awk '{print $1}')" == "${V3_SHA256}" ]] || {
    echo "Rare-tuned V3 checkpoint SHA256 mismatch" >&2
    exit 1
}
[[ ! -e "${RUN_ROOT}" ]] || { echo "Audit root already exists: ${RUN_ROOT}" >&2; exit 1; }
mkdir -p "${RUN_ROOT}"
LOG="${RUN_ROOT}/audit.log"
exec > >(tee -a "${LOG}") 2>&1
CURRENT_STAGE=preflight
trap 'rc=$?; echo "FAIL hard-pair audit stage=${CURRENT_STAGE} exit=${rc} log=${LOG}" >&2' ERR
...
if not any(name in names[0].upper() for name in ("H100", "H200")):
    raise RuntimeError(f"hard-pair audit requires Hopper, got {names[0]!r}")
...
CURRENT_STAGE=locked_top5_audit
PYTHONPATH="${SCRIPT_DIR}:${PYTHONPATH}" "${ALGENGINE_PYTHON}" \
    "${SCRIPT_DIR}/probe_hard_pair_information.py" \
    ...
    --prior-interaction-gate "${PRIOR_GATE}" \
    --output "${RUN_ROOT}/hard_pair_gate.json" \
    --device cuda

CURRENT_STAGE=complete
trap - ERR
echo "PASS hard-pair train-only audit: ${RUN_ROOT}"
echo "gate: ${RUN_ROOT}/hard_pair_gate.json"
echo "log: ${LOG}"
