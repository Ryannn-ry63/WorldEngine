#!/usr/bin/env python3
"""Shortlist selector architectures using only log-disjoint rare development tokens."""
...
def main():
    ...
    parser.add_argument("--expected-candidates", type=int, default=15)
    ...
    if args.expected_candidates <= 0 or args.shortlist_size <= 0:
        raise ValueError("candidate and shortlist counts must be positive")
    ...
    if not evaluations:
        raise RuntimeError("no trajectory-set development evaluations found")
    candidates = []
    ...
    if len(candidates) != args.expected_candidates:
        raise RuntimeError(
            f"architecture candidate count {len(candidates)} "
    ...
    if not eligible and args.require_positive_gate:
        raise RuntimeError("no architecture checkpoint passed cache-development gate")
    ...
    if common.sha256_file(state_path) != selected["scene_selector_state_sha256"]:
        raise RuntimeError("selected architecture state SHA256 drifted")
    report = {
        "schema_version": 1,
        "status": "PASS",
        "method": "trajectory_set_reasoner_development_shortlist_v1",
        ...
        "selected": selected,
        "shortlist": ranked[: args.shortlist_size],
        "closed_loop_promotion_required": True,
        "cache_development_result_is_not_a_formal_promotion": True,
    }
