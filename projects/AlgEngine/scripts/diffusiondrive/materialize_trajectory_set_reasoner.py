#!/usr/bin/env python3
"""Materialize a schema-v4 trajectory-set selector into epoch-100 baseline."""
...
def build_parser():
    ...
    parser.add_argument(
        "--expected-method", required=True
    )
    parser.add_argument(
        "--release-name", default="e2e_diffusiondrive_trajectory_set_reasoner_v1"
    )
    return parser
...
def main():
    ...
    if selector_sha != args.expected_selector_sha256:
        raise RuntimeError("trajectory-set selector SHA256 mismatch")
    selector_payload = torch.load(selector_path, map_location="cpu")
    if selector_payload.get("schema_version") != 4:
        raise RuntimeError("trajectory-set selector payload schema drifted")
    if selector_payload.get("method") != args.expected_method:
        raise RuntimeError("trajectory-set selector method drifted")
    ...
    checkpoint.setdefault("meta", {})["trajectory_set_reasoner_grpo_v1"] = {
