...
RUN_ID="${RARE_LOG_SMOKE_ID:-smoke_$(date -u +%Y%m%dT%H%M%SZ)}"
...
ROOT="${WORLDENGINE_ROOT}/experiments/diffusiondrive/grpo_selector_v3_rare_log_v1/local_smoke/${RUN_ID}"
SUBSET="${ROOT}/subset"
MINING="${ROOT}/mining"
...
SUBSET_TOOL="${SCRIPT_DIR}/prepare_grpo_selector_v3_rare_log_smoke_subset.py"
...
trap 'rc=$?; echo "FAIL one-H100 rare-log smoke stage=${CURRENT_STAGE} line=${LINENO} command=${BASH_COMMAND} exit=${rc}" >&2; echo "persistent_log: ${LOG_FILE}" >&2' ERR
...
for path in "${CONFIG}" "${BASELINE}" "${FULL_FILTER}" "${NAVTEST_FILTER}" \
    ...
    "${PLANNING_HEAD}" "${SCENE_SELECTOR}" "${NAVFORMER}" "${TEST_SCRIPT}"; do
...
"${ALGENGINE_PYTHON}" "${SCRIPT_DIR}/materialize_grpo_selector_v3.py" \
    ...
    --expected-method "scene_conditioned_exact_group_grpo_v3_rare_log_v1" \
    --release-name "e2e_diffusiondrive_grpo_selector_v3_rare_log_v1_smoke" \
...
run_inference 0 "${ROOT}/checkpoint.pth" "rare_log_smoke_trained_eval" \
...
import prepare_grpo_selector_v3_rare_log_data as prepare
...
trap - ERR
echo "PASS one-H100 V3 rare-log end-to-end smoke"
echo "output: ${ROOT}"
