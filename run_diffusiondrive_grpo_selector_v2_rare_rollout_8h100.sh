#!/usr/bin/env bash
# V2 rare-rollout: reuse immutable epoch-100 synthetic data; never recollect.
...
V3_ROLLOUT="${WORLDENGINE_ROOT}/experiments/diffusiondrive/grpo_selector_v3_rare_rollout_v1"
DATA_ROOT="${V3_ROLLOUT}/data"
REAL_CACHE="${WORLDENGINE_ROOT}/experiments/diffusiondrive/grpo_selector_v3_rare_original_v1/cache/full"
ROOT="${WORLDENGINE_ROOT}/experiments/diffusiondrive/grpo_selector_v2_rare_rollout_v1"
SWEEP_ROOT="${ROOT}/sweep"
CERT_ROOT="${ROOT}/certification"
MODEL_ROOT="${ROOT}/models"
FORMAL_ROOT="${ROOT}/formal"
LOG_DIR="${ROOT}/logs"
TRAINER="${SCRIPT_DIR}/train_grpo_selector_v2_cached_rare_rollout.py"
EVALUATOR="${SCRIPT_DIR}/evaluate_grpo_selector_v2_rare_rollout.py"
SELECTOR="${SCRIPT_DIR}/select_grpo_selector_v2_rare_rollout.py"
CERTIFIER="${SCRIPT_DIR}/certify_grpo_selector_v2_rare_rollout.py"
...
trap 'rc=$?; printf "FAIL mode=%s seed=%s stage=%s exit=%s\n" "${MODE}" "${SEED:-none}" "${CURRENT_STAGE}" "${rc}" > "${STATUS}"; echo "FAIL V2 rare-rollout stage=${CURRENT_STAGE} exit=${rc}" >&2; echo "persistent_log: ${LOG_FILE}" >&2' ERR
...
run_preflight() {
    ...
    for path in "${BASELINE}" "${DIFFUSIONDRIVE_GRPO_CONFIG}" "${TRAINER}" \
        ...
        "${DATA_ROOT}/manifest.json" "${DATA_ROOT}/hard_pool.jsonl" \
        "${DATA_ROOT}/synthetic_cache.pt"; do
        [[ -f "${path}" ]] || { echo "Missing input: ${path}" >&2; exit 1; }
    done
    for seed in 0 1 2; do
        [[ -f "${REAL_CACHE}/train_seed${seed}/cache.pt" ]] || {
            echo "Missing real cache seed ${seed}" >&2; exit 1;
        }
    done
    ...
    if ! git -C "${WORLDENGINE_ROOT}" diff --quiet -- projects/AlgEngine projects/SimEngine; then
        echo "Tracked implementation files are dirty" >&2; exit 1
    fi
    "${ALGENGINE_PYTHON}" -c '
...
assert x["status"]=="PASS" and x["source_policy"]=="immutable_epoch100_diffusiondrive"
assert x["behavior_policy_is_trained_v3"] is False
assert x["mixture_contract"]["overall_common_fraction"]==0.5
assert x["mixture_contract"]["overall_hard_fraction"]==0.5
assert sha(pathlib.Path(x["synthetic_cache"]))==x["synthetic_cache_sha256"]
assert sha(pathlib.Path(x["hard_pool"]))==x["hard_pool_sha256"]
' "${DATA_ROOT}/manifest.json"
    ...
    echo "PASS V2 shared rare-rollout data preflight (no recollection)"
}

trial_ready() {
    [[ -f "$1/report.json" && -f "$1/development_evaluation.json" ]] || return 1
    json_pass "$1/report.json"; json_pass "$1/development_evaluation.json"
}
...
run_trial() {
    ...
    mkdir -p "${trial}"
    if trial_ready "${trial}"; then echo "SKIP verified rollout trial ${name}"; return; fi
    CUDA_VISIBLE_DEVICES="${gpu}" "${ALGENGINE_PYTHON}" "${TRAINER}" \
        ...
        --hard-pool-split train --output-dir "${trial}" \
        --temperature "${temperature}" --learning-rate "${learning_rate}" \
        --kl-weight "${kl_weight}" --seed 0 --epochs 64 \
        --examples-per-cache-epoch 6339 --checkpoint-epochs 1,2,4,8,16,32,64 \
        --batch-size 64 --device cuda --method-name exact_group_grpo_v2_rare_rollout_tuning_v1
    CUDA_VISIBLE_DEVICES="${gpu}" "${ALGENGINE_PYTHON}" "${EVALUATOR}" \
        ...
        --synthetic-cache "${DATA_ROOT}/synthetic_cache.pt" \
        --data-manifest "${DATA_ROOT}/manifest.json" --hard-pool "${DATA_ROOT}/hard_pool.jsonl" \
        --split development --output "${trial}/development_evaluation.json" \
...
run_tune() {
    ...
    for gpu in 0 1 2 3 4 5 6 7; do
        run_trial "${gpu}" "${temperatures[${gpu}]}" \
            ...
            > "${LOG_DIR}/tune_gpu${gpu}.log" 2>&1 & pids+=("$!")
    done
    local failed=0
    for pid in "${pids[@]}"; do wait "${pid}" || failed=1; done
    [[ "${failed}" -eq 0 ]] || { echo "V2 rollout sweep failed" >&2; return 1; }
    ...
    echo "PASS V2 rollout independent tuning and certification"
}
...
training_ready() {
    local family="$1" seed="$2" method="$3" epoch="$4" root="${MODEL_ROOT}/${1}/seed${2}"
...
train_family() {
    ...
    local kl_weight="$6" epochs="$7" method="$8" root="${MODEL_ROOT}/${1}/seed${2}"
    if training_ready "${family}" "${seed}" "${method}" "${epochs}"; then
        echo "SKIP verified ${family} seed ${seed}"; return
    fi
    [[ ! -e "${root}" ]] || { echo "Partial immutable root exists: ${root}" >&2; return 1; }
    mkdir -p "${root}/train"
    local formal_args=()
    [[ "${family}" == "rare_rollout_fixed" ]] && formal_args+=(--formal-contract)
    CUDA_VISIBLE_DEVICES="${gpu}" "${ALGENGINE_PYTHON}" "${TRAINER}" \
        --real-cache "0=${REAL_CACHE}/train_seed0/cache.pt" \
        --real-cache "1=${REAL_CACHE}/train_seed1/cache.pt" \
        --real-cache "2=${REAL_CACHE}/train_seed2/cache.pt" \
        --synthetic-cache "${DATA_ROOT}/synthetic_cache.pt" \
        --data-manifest "${DATA_ROOT}/manifest.json" --hard-pool "${DATA_ROOT}/hard_pool.jsonl" \
        --hard-pool-split all --output-dir "${root}/train" \
        --temperature "${temperature}" --learning-rate "${learning_rate}" \
        --kl-weight "${kl_weight}" --seed "${seed}" --epochs "${epochs}" \
        --examples-per-cache-epoch 6339 --checkpoint-epochs "${epochs}" \
        --batch-size 64 --device cuda --method-name "${method}" "${formal_args[@]}"
...
evaluation_ready() {
    local model="e2e_diffusiondrive_grpo_selector_v2_${1}_s${2}"
    [[ -f "${FORMAL_ROOT}/formal_eval/${model}/summary.json" ]] || return 1
    json_pass "${FORMAL_ROOT}/formal_eval/${model}/summary.json"
}

evaluate_family() {
    local family="$1" seed="$2" root="${MODEL_ROOT}/${1}/seed${2}"
    if evaluation_ready "${family}" "${seed}"; then echo "SKIP eval ${family} seed ${seed}"; return; fi
    local sha="$(sha256sum "${root}/checkpoint.pth" | awk '{print $1}')"
    ...
    "${FORMAL_TABLE}" "${root}/checkpoint.pth" "${sha}" \
        "e2e_diffusiondrive_grpo_selector_v2_${family}_s${seed}" "${seed}" \
        "V2 corrected rare-rollout family=${family}; shared epoch100 synthetic data"
...
run_formal_seed() {
    ...
    if [[ "${SEED}" != "0" ]]; then
        ...
        train_family rare_rollout_fixed "${SEED}" 0 1 1e-4 1e-3 16 \
            exact_group_grpo_v2_rare_rollout_v1 \
            > "${LOG_DIR}/train_fixed_s${SEED}.log" 2>&1
        CURRENT_STAGE=evaluate_fixed
        evaluate_family rare_rollout_fixed "${SEED}"
        CURRENT_STAGE=wait_for_seed0_tuning
        wait_for_tuning
        readarray -t tuned < <(selected_values)
        CURRENT_STAGE=train_tuned
        train_family rare_rollout_tuned "${SEED}" 0 "${tuned[0]}" "${tuned[1]}" \
            "${tuned[2]}" "${tuned[3]}" exact_group_grpo_v2_rare_rollout_tuned_v1 \
            > "${LOG_DIR}/train_tuned_s${SEED}.log" 2>&1
        CURRENT_STAGE=evaluate_tuned
        evaluate_family rare_rollout_tuned "${SEED}"
        echo "PASS V2 rare-rollout coordinated fixed+tuned formal seed ${SEED}"
        return
    fi
    ...
    train_family rare_rollout_fixed "${SEED}" 0 1 1e-4 1e-3 16 \
        exact_group_grpo_v2_rare_rollout_v1 > "${LOG_DIR}/train_fixed_s${SEED}.log" 2>&1 &
    local fixed_pid=$!
    train_family rare_rollout_tuned "${SEED}" 1 "${tuned[0]}" "${tuned[1]}" \
        "${tuned[2]}" "${tuned[3]}" exact_group_grpo_v2_rare_rollout_tuned_v1 \
        > "${LOG_DIR}/train_tuned_s${SEED}.log" 2>&1 &
    local tuned_pid=$!
    local failed=0
    ...
    [[ "${failed}" -eq 0 ]] || { echo "V2 rollout training failed" >&2; return 1; }
    CURRENT_STAGE=evaluate_fixed
    evaluate_family rare_rollout_fixed "${SEED}"
    CURRENT_STAGE=evaluate_tuned
    evaluate_family rare_rollout_tuned "${SEED}"
...
find_v2_common() {
    local seed="$1"
    find "${WORLDENGINE_ROOT}/experiments/diffusiondrive/grpo_selector_v2_progress_fix_v1/formal/formal_eval" \
...
run_summary() {
    ...
    for seed in 0 1 2; do
        ...
        mapfile -t common_matches < <(find_v2_common "${seed}")
        [[ "${#common_matches[@]}" -eq 1 ]] || return 1
        args+=(--input "v2_common_progress_fixed:${seed}=${common_matches[0]}")
        args+=(--input "rare_original_fixed:${seed}=${WORLDENGINE_ROOT}/experiments/diffusiondrive/grpo_selector_v2_rare_original_v1/formal/formal_eval/e2e_diffusiondrive_grpo_selector_v2_rare_original_fixed_s${seed}/summary.json")
        args+=(--input "rare_original_tuned:${seed}=${WORLDENGINE_ROOT}/experiments/diffusiondrive/grpo_selector_v2_rare_original_v1/formal/formal_eval/e2e_diffusiondrive_grpo_selector_v2_rare_original_tuned_s${seed}/summary.json")
        args+=(--input "rare_rollout_fixed:${seed}=${FORMAL_ROOT}/formal_eval/e2e_diffusiondrive_grpo_selector_v2_rare_rollout_fixed_s${seed}/summary.json")
        args+=(--input "rare_rollout_tuned:${seed}=${FORMAL_ROOT}/formal_eval/e2e_diffusiondrive_grpo_selector_v2_rare_rollout_tuned_s${seed}/summary.json")
    done
    "${ALGENGINE_PYTHON}" "${SUMMARIZER}" "${args[@]}" \
        --provenance "${DATA_ROOT}/manifest.json" \
        --provenance "${SWEEP_ROOT}/selection.json" \
        --provenance "${CERT_ROOT}/report.json" \
        --output "${FORMAL_ROOT}/v2_rare_rollout_comparison.json"
}
...
trap - ERR
echo "PASS V2 rare-rollout mode=${MODE}${SEED:+ seed=${SEED}}"
echo "status: ${STATUS}"
